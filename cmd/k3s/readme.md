...
| k3s组件 | Linux软链接 | k3s | crictl, ctr, k3s, k3s-agent, k3s-certificate, k3s-etcd-snapshot, k3s-secrets-encrypt, k3s-server, kubectl |
| k3s网络插件 | Linux软链接 | cni | bridge, cni, flannel, host-local, loopback, portmap | 
| k3s-containerd运行时 | 磁盘文件 | - | conntrack, containerd, containerd-shim-runc-v2, check-config |
| 其它包 | 磁盘文件 | - | aux, blkid,  ethtool, find, fuse-overlayfs, ip, ipset, losetup, nsenter, runc, slirp4netns, swanctl, charon, swanctl |